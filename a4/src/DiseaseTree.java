/* NetId(s): cxa2, nnnnn. Time spent: 06 hours, 20 minutes.

 * Name(s): Cristian Alonso
 * What I thought about this assignment:
 * Slightly lengthy, but fun and interesting.
 *
 */
...
public class DiseaseTree {
    ...
    public DiseaseTree add(Person p, Person c) throws IllegalArgumentException {
        //TODO 1
    	if (p==null) {
    		throw new IllegalArgumentException("Cannot add " + c.getName() + " as child of null.");
    	}
    	else if (c==null){
    		throw new IllegalArgumentException("Cannot add null to " + p.getName());
    	}
    	if (!contains(c)) {
    		DiseaseTree dt = getTree(p);
    		if (dt != null) {
    			dt = dt.getTree(p);
    			DiseaseTree childTree = new DiseaseTree(c);
    			dt.children.add(childTree);
    			return childTree;
    		}
    		else throw new IllegalArgumentException(p.getName() + " is not in this DiseaseTree");
    	}
    	else throw new IllegalArgumentException(c.getName() + " is already in this DiseaseTree");
    }
    ...
    public int widthAtDepth(int d) throws IllegalArgumentException {
        //TODO 4
    	int count = 0;
        if (d < 0) throw new IllegalArgumentException("Depth must be 0 or positive integer");
        if (d == 0) return 1;
        for (DiseaseTree dt : children) {
        	count = count + dt.widthAtDepth(d-1);
        }
        return count;
    }
    ...
    public List<Person> getDiseaseRouteTo(Person c){
        ...
        for (DiseaseTree dt : children) {
        	if (dt.contains(c)) {
        		pList.addAll(dt.getDiseaseRouteTo(c));
        	}
        }
        return pList;
    }
    ...
    public Person getSharedAncestorOf(Person child1, Person child2) {
    	...
    	if (getTree(getParentOf(child1)).contains(child2)){
    		return getParentOf(child1);
    	}
    	if (getTree(getParentOf(child2)).contains(child1)){
    		return getParentOf(child2);
    	}
    	int depth1 = depthOf(child1);
    	int depth2 = depthOf(child2);
    	if (depth1 < depth2) {
    		return getSharedAncestorOf(getParentOf(child1), child2);
    	}
    	else {
    		return getSharedAncestorOf(child1, getParentOf(child2));
    	}
    ...
    public boolean equals(Object ob){
        ...
        if ((getRoot() != ((DiseaseTree) ob).getRoot())) return false;
        else {
        	if (getChildrenCount() != ((DiseaseTree) ob).getChildrenCount()) return false;
        	else {
        		for (DiseaseTree dt : children) {
        			if (!equalsAChild(dt, ((DiseaseTree) ob).children)) {
        				return false;
        			}
        		}
        	}
