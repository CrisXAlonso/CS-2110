...
public class TreeMap {
    ...
    public static Wrapper2 getSplit(List<Node> b, int m, int n) {
        ...
        if (totWeight <= 0) {
        	...
        	h = m;
        	double splitWeight = b.get(h).getWeight();
        	while (splitWeight < SPLIT_RATIO*totWeight) {
        		h++; 
        		splitWeight += b.get(h).getWeight();
        	}
        	splitRatio = splitWeight/totWeight;
        }
        Wrapper2 wrap = new Wrapper2(h,splitRatio);
        return wrap;
    }
    ...
    private void sliceAndDice(List<Node> b, int m, int n,
    	...
    	if (b.subList(m, n+1).size()==0) return;
    	if (b.subList(m, n+1).size()==1) {
    		b.get(m).block = new Block(bbox, new Color (0, 0, 127));
    		return;
    	}
    	Wrapper2 wrap = getSplit(b, m, n);
    	int k = wrap.k;
    	double splitRatio = wrap.d;
    	
    	double bboxWidth = w*bbox.getWidth();
    	double bboxHeight = h*bbox.getHeight();
    	BoundingBox bbox1;
    	BoundingBox bbox2;
    	if (bboxWidth > bboxHeight) {
    		double splitX = (bbox.high.x-bbox.low.x)*splitRatio;
    		bbox1 = new BoundingBox(bbox);
    		bbox2 = new BoundingBox(bbox);
    		bbox1.high.x = bbox.low.x+splitX;
    		bbox2.low.x = bbox.low.x+splitX;
    	}
    	else {
    		double splitY = (bbox.high.y-bbox.low.y)*splitRatio;
    		bbox1 = new BoundingBox(bbox);
    		bbox2 = new BoundingBox(bbox);
    		bbox1.high.y = bbox.low.y+splitY;
    		bbox2.low.y = bbox.low.y+splitY;
    	}
    	
    	sliceAndDice(b, m, k, bbox1, w, h);
    	sliceAndDice(b, k+1, n, bbox2, w, h);
    }
    ...
    public static void main(String[] args) {
        ...
        ((TreeMap)ftmap).printTree();
        ((TreeMap)ftmap).paint(jf.getGraphics(), 300, 300);
    }
}
